...
class AdminUserSignup extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ->greeting('New user!')
                    ->line($this->user->email.' has signed up.');
    }

    /**
     * Issue a Spark Notification
     *
     * @param $notifiable
    ...
    public function toSpark($notifiable)
    {
        return (new SparkNotification)
                    ->icon('fa-users')
                    ->body('A new user ('.$this->user->email.') signed up!');
    }
